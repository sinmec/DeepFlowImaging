...
def generate_validation_data(imgs, bbox_datasets, img_size, N_SUB, N_ANCHORS, ANCHOR_SIZES, ANCHOR_RATIOS, N_RATIOS,
    ...
    print(f"Number of validation images: {N_validation}")

    # Initializing the arrays
    batch_imgs = np.zeros((N_validation, img_size[0], img_size[1], 1), dtype=np.float32)
    ...
    batch_anchor_locations = np.zeros(
        (N_validation, img_size[0] // N_SUB, img_size[1] // N_SUB, 4 * N_ANCHORS * N_RATIOS), dtype=np.float32)

    for index, (img, bbox_dataset) in enumerate(tqdm(zip(imgs, bbox_datasets), desc="Generating validation data")):
        ...
        labels, anchor_argmax_ious = evaluate_ious(anchors, index_anchors_valid, ious, bbox_dataset, img,
                                                   POS_IOU_THRESHOLD, NEG_IOU_THRESHOLD, debug=DEBUG)
        ...
        if anchor_locations.ndim == 1:
            anchor_locations = anchor_locations.reshape(
                (img_size[0] // N_SUB, img_size[1] // N_SUB, 4 * N_ANCHORS * N_RATIOS))

        for dim in range(4):
            anchor_locations[:, :, dim::4] /= img_size[dim % 2]

...
def create_samples_for_training(anchors, index_anchors_valid, anchor_argmax_ious, labels, ious, bbox_dataset, image, debug=False):
    ...
    # When labelling the images, most of the pixel values correspond to background.
    ...
    # labels defined as -1 are not used during training
    ...
    if cfg.SHOW_N_POS:
        print('n_posi = %d / %d(set)' % (np.sum(labels == 1), cfg.POS_RATIO * cfg.N_SAMPLES))
    ...
    if debug:
        ...
        for k in range(len(labels)):
            ...
            #     p_1 = (x_a_1, y_a_1)
            #     p_2 = (x_a_2, y_a_2)
            #     cv2.rectangle(image_rgb, p_1, p_2, (0,0,255), 4) #RED
